...
public abstract class AbstractNewInjectingActivity extends Activity {
		
    private static final Logger logger = LoggerFactory.getLogger(AbstractNewInjectingActivity.class);
    
    private final CallbackFinalizer finalizer = new DefaultCallbackFinalizer();
    
    protected final Executor nonUIThread = Executors.newSingleThreadExecutor();
    ...
    private void connectThings() {
        ...
        if(displays == null) {
            ...
            final Set<Class<?>> controllerDisplayClasses = new HashSet<Class<?>>();
            
            for(NewInjectableController<?> controller : controllers) {
                controllerDisplayClasses.add(controller.displayClass);
            }
                
            for(Class<?> clazz : controllerDisplayClasses) {
                for(final InjectingDisplay display : getDisplays()) {
                    
                    if(clazz.isInstance(display)) {
